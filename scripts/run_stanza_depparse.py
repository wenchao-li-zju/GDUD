#!/usr/bin/env python3
"""Run one Stanza dependency parser train+predict experiment.

This wrapper intentionally streams Stanza logs to the terminal so training
...
def score_parser(gold_path: Path, pred_path: Path) -> dict[str, float | int]:
...
def build_common_parser_args(args: argparse.Namespace) -> list[str]:
    return [
        "--lang",
        args.lang,
        "--shorthand",
        ...
        str(args.deep_biaff_output_dim),
    ]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one Stanza dependency parser train+predict experiment."
    )
    ...
    parser.add_argument("--save-name", default="parser.pt")
    ...
    parser.add_argument("--deep-biaff-output-dim", default=160, type=int)
...
def main() -> int:
    ...
    common = build_common_parser_args(args)
    ...
    scores = score_parser(args.test_file, test_pred)
    row = {
    ...
    print(
        ...
        f"UPOS={row['upos']} UAS={row['uas']} LAS={row['las']} "
        f"tokens={row['tokens']}"
    )
