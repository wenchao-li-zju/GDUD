#!/usr/bin/env python3
"""Run UPOS transfer experiments: Nanai -> Ulch/Negidal/Oroch."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Nanai UPOS transfer experiments.")
...
def main() -> int:
    ...
    if unknown_targets:
        raise SystemExit(
            "Unknown UPOS transfer target(s): "
    ...
    print(f"Selected {len(jobs)} UPOS transfer prediction job(s).", flush=True)
    print(f"Training jobs implied: {len(folds)} Nanai source tagger model(s).", flush=True)
    ...
    for index, (fold, target) in enumerate(jobs, start=1):
        ...
        model_dir = root / "models_upos" / "transfer" / f"{SOURCE_LANGUAGE}_to_targets" / f"fold{fold}"
        pred_dir = root / "predictions_upos" / "transfer" / f"{SOURCE_LANGUAGE}_to_{target}" / f"fold{fold}"
        shorthand = f"gdud_{SOURCE_LANGUAGE.lower()}_transfer_upos"
        ...
        print(
            f"\n=== [{index}/{len(jobs)}] UPOS {SOURCE_LANGUAGE} transfer fold{fold} -> {target} ===",
            flush=True,
        )
